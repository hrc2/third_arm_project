...
m1plot = np.absolute(np.array(read_data('motor1_brace_filtered.csv')).astype(float))
m2plot = np.absolute(np.array(read_data('motor2_brace_filtered.csv')).astype(float))
x = np.linspace(0,len(m1plot)/10000.0,num=len(m1plot))

fig1 = plt.figure()
fig1.suptitle('Torque Load on Motors while Bracing',fontsize=20)
...
ax1.set_ylabel('Ratio of Torque to Peak Motor Torque',fontsize=14)
ax1.legend(loc=2)
#ax1.set_xticks([])
plt.show()
