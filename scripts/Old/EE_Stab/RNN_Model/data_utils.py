    ...
    for subj in subjects:
        for action_idx in np.arange(len(actions)):
            ...
            for subact in [1]:  # subactions
                ...
                if subj == 1:
                    filename = path_to_dataset + '/wrf_train.csv'
                elif subj == 2:
                    filename = path_to_dataset + '/wrf_valid.csv'
...
def normalization_stats(completeData):
    ...
    dimensions_to_ignore.extend( list(np.where(data_std < 1e-6)[0]) )
    dimensions_to_use.extend( list(np.where(data_std >= 1e-6)[0]) )
