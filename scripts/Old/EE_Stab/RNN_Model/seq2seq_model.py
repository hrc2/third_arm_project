...
class Seq2SeqModel(object):
    ...
    def get_batch_srnn(self, data, action ):
        ...
        actions = ["directions", "discussion", "eating", "greeting", "phoning",
                   ...
                   "takingphoto", "waiting", "walking", "walkingdog", "walkingtogether", "wrf"]
        ...
        batch_size = 4 # we always evaluate 8 seeds
        subject    = 2 # we always evaluate on subject 5
        source_seq_len = self.source_seq_len
        target_seq_len = self.target_seq_len

        # seeds = [( action, (i%2)+1, frames[action][i] ) for i in range(batch_size)]
        seeds = [( action, 1, frames[action][i] ) for i in range(batch_size)]
