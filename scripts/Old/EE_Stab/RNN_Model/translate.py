...
tf.app.flags.DEFINE_string("action", "wrf", "The action to train on. all means all the actions, all_periodic means walking, eating and smoking")
        ...
        if current_step % FLAGS.test_every == 0:
            ...
            for action in actions:
                ...
                for i in np.arange(N_SEQUENCE_TEST):
                    ...
                    idx_to_use = np.where( np.std( gt_i, 0 ) > 1e-6 )[0]
            ...
            print("============================\n"
                  ...
                  "Val loss:            %.4f\n"
                  "srnn loss:           %.4f\n"
                  "============================" % (model.global_step.eval(session=sess),
                                                    model.learning_rate.eval(session=sess), step_time*1000, loss,
                                                    val_loss, srnn_loss))
            print()
...
def get_srnn_gts( actions, model, test_set, data_mean, data_std, dim_to_ignore, one_hot, to_euler=False ):
    """
